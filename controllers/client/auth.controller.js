...
module.exports.registerPost = async (req, res) => {
    ...
    if (!fullName || !email || !password || !confirmPassword || !phone || !address || !username || !gender || !dob) {
    ...
    try {
        ...
        const existingUsername = await User.findOne({ username, deleted: false });
        if (existingUsername) {
        ...
        // Liên kết giỏ hàng với người dùng (nếu có)
        if (req.cookies.cartId) {
            await Cart.updateOne({ _id: req.cookies.cartId }, { user_id: newUser._id });
        }
