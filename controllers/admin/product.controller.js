...
module.exports.editPatch = async (req, res) => {
  try {
    ...
    const { price, discountPercentage, stock, position,rating, ...rest } = req.body;
    ...
    const sanitizedData = {
      ...
      rating: !isNaN(parseFloat(rating)) ? parseFloat(rating) : 1,
      position: position ? (!isNaN(parseInt(position)) ? parseInt(position) : 1) : product.position,
...
module.exports.changeIsBestSeller = async (req, res) => {
  try {
    ...
    // Đảo ngược giá trị isBestSeller
    product.isBestSeller = !product.isBestSeller;

    // Lưu lại thay đổi
    await product.save();

    req.flash('success', `Thay đổi thành ${product.isBestSeller ? 'Nổi bật' : 'Không nổi bật'}.`);
    return res.redirect('back');
  } catch (error) {
    console.error('❌ Lỗi khi thay đổi trạng thái isBestSeller:', error);
    req.flash('error', 'Có lỗi xảy ra khi thay đổi trạng thái "Nổi bật".');
    return res.redirect('back');
  }
};
